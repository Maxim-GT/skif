  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 text-center z-10 mb-16">
        ...
        >
          <img src="./logo3.webp" alt="Логтип Скриф" className="w-[5rem] lg:w-[12rem] sm:mb-5 mb-0"/>
          ...
          <p 
            className={`text-lg md:text-xl text-white/90 max-w-3xl mx-auto mb-8 transition-all duration-1000 delay-300 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
            Профессиональный стрелковый клуб в Абакане –
            искусство стрельбы в безопасной и комфортной обстановке
          </p>
          
          <div 
            className={`flex flex-col sm:flex-row items-center justify-center gap-4 transition-all duration-1000 delay-500 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
      ...
      <div 
        className={`flex absolute transform-translate-x-1/2 cursor-pointer animate-bounce transition-all duration-2500 delay-1000 bottom-[3rem] lg:bottom-[2rem] ${isLoaded ? 'opacity-100' : 'opacity-0'} no-custom-cursor`}
        onClick={scrollToAbout}
      >
        <ArrowDown className="text-white h-36 lg:h-24 w-10" />
      </div>
    </section>
  );
};
